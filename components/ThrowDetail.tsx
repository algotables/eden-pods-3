...
export default function ThrowDetail({ throwData }: { throwData: UnifiedThrow }) {
  ...
  const [harvestModal, setModal]    = useState(false);
  ...
  const [harvestOnChain, setHarvestOnChain] = useState(true);

  const pt    = POD_TYPES.find((p) => p.id === throwData.podTypeId);
  const model = GROWTH_MODELS.find((m) => m.id === throwData.growthModelId);

  const stageData = model ? getCurrentStage(throwData.throwDate, model) : null;
  const nextStage = model ? getNextStage(throwData.throwDate, model) : null;
  ...
  // Combine on-chain and local harvests for this throw
  const myOnChainHarvests = onChainHarvests.filter((h) => h.throwAsaId === throwData.asaId);
  const myLocalHarvests   = localHarvests.filter(
  ...
  const logHarvest = async () => {
    ...
    try {
      if (harvestOnChain && throwData.asaId > 0) {
        // On-chain harvest record
        ...
        toast.success("Harvest logged locally! 🥗");
      }
  ...
  const totalGrams =
    myOnChainHarvests.reduce((sum, h) => sum + QUANTITY_GRAMS[h.quantityClass], 0) +
    myLocalHarvests.reduce((sum, h) => sum + QUANTITY_GRAMS[h.quantityClass], 0);

  if (!pt || !model || !stageData) {
    return (
  ...
  return (
    <div className="pb-6">
      ...
      >
        ...
        {throwData.asaId === 0 && (
          <div className="mb-3">
            <span className="inline-flex items-center gap-1.5 text-xs bg-white/20 text-white px-3 py-1.5 rounded-full">
        ...
        <div className="bg-white/15 rounded-2xl px-4 py-3">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-white/80 text-sm">Day {stageData.daysSince}</p>
            </div>
            {nextStage && (
        ...
        <div className="px-4 mt-4">
          <div className="relative">
            ...
            <div className="space-y-4">
              {model.stages.map((s) => {
                const isDone = stageData.daysSince >= s.dayEnd;
                const isCur  = s.id === stageData.stage.id;
                const isFut  = stageData.daysSince < s.dayStart;
                return (
                  <div key={s.id} className="flex gap-4">
                    ...
                    <div className={cn("flex-1 pb-4", isFut && "opacity-40")}>
                      <div className={cn(
                        ...
                        isCur ? "bg-eden-50 border-eden-200" : "bg-white border-gray-100"
                      )}>
                        <div className="flex items-center justify-between mb-1">
                          <span className={cn("font-semibold", isCur ? "text-eden-800" : "text-gray-700")}>
                            {s.name}
                          </span>
                        ...
                        {isCur && (
                          <div className="bg-eden-100 text-eden-800 rounded-xl px-3 py-1 text-xs font-medium mb-2">
                            ← You are here
                          </div>
                        )}
          ...
          {myObs.length > 0 && (
            <div className="mt-4">
              <h3 className="font-semibold text-gray-700 mb-2">📋 Observations (local)</h3>
        ...
        <div className="px-4 mt-4 space-y-4">
          ...
          {throwData.asaId > 0 && (
            <div className="flex items-center gap-3 p-3 bg-purple-50 border border-purple-200 rounded-2xl">
              ...
              <button
                onClick={() => setHarvestOnChain((v) => !v)}
                className={cn(
                  "w-12 h-6 rounded-full transition-colors relative",
                  harvestOnChain ? "bg-purple-600" : "bg-gray-300"
                )}
              >
                <span className={cn(
                  "absolute top-0.5 w-5 h-5 bg-white rounded-full shadow transition-transform",
                  harvestOnChain ? "translate-x-6" : "translate-x-0.5"
                )} />
              </button>
            </div>
          )}
          ...
          >
            🥗 Log a Harvest {harvestOnChain && throwData.asaId > 0 ? "(on-chain)" : "(local)"}
          </button>

          {/* On-chain harvests */}
          {myOnChainHarvests.length > 0 && (
            <div>
              <h3 className="font-semibold text-gray-700 mb-2 flex items-center gap-2">
                <span>◈</span> On-chain harvests
              </h3>
              <div className="space-y-2">
                {myOnChainHarvests.map((h) => (
          ...
          {/* Local harvests */}
          {myLocalHarvests.length > 0 && (
            <div>
              <h3 className="font-semibold text-gray-700 mb-2">💾 Local harvests</h3>
              <div className="space-y-2">
                {myLocalHarvests.map((h) => (
          ...
          {myOnChainHarvests.length === 0 && myLocalHarvests.length === 0 && (
            <div className="text-center py-8 text-gray-400">
              <div className="text-4xl mb-2">🥗</div>
              <p className="text-sm">No harvests yet</p>
            </div>
          )}
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-end justify-center">
          <div className="bg-white rounded-t-3xl p-6 w-full max-w-lg max-h-[85vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-5">
              ...
              <button onClick={() => setModal(false)} className="text-gray-400 text-2xl hover:text-gray-600">×</button>
            </div>
            <div className="space-y-5">
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="e.g. Young leaves, very tender"
                  rows={2}
              ...
              >
                {savingH ? "Saving…" : harvestOnChain && throwData.asaId > 0 ? "◈ Record On-Chain" : "Save Locally"}
              </button>
            </div>
          </div>
        </div>
