...
export default function NotifCenter() {
  ...
  const all = notifications.slice(0, 30);
    ...
    <div className="px-4 py-6 fade-up">
        ...
        <div className="flex gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-blue-600 mt-0.5">
              Notifications are generated automatically when you log a throw.
              They appear here when each growth stage window opens.
            </p>
          </div>
        </div>
      </div>
      ...
      {all.length === 0 ? (
        <div className="text-center py-16">
          ...
          <p className="text-gray-500 text-sm">Stage alerts will appear here as your pods grow.</p>
        </div>
      ) : (
        <div className="space-y-3">
          {all.map((n) => {
            const isDue = !n.read && n.scheduledFor <= new Date().toISOString();
            return (
              <div key={n.id}
                className={cn("rounded-2xl p-4 border transition-all",
                  isDue ? "bg-white border-eden-200 shadow-sm" : "bg-white border-gray-100 opacity-60"
                )}>
                <div className="flex items-start gap-3">
                  <div className={cn("w-12 h-12 rounded-2xl flex items-center justify-center text-2xl flex-shrink-0",
                    isDue ? "bg-eden-100" : "bg-gray-100")}>
                    {n.stageIcon}
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      <p className="font-semibold text-gray-900 text-sm">{n.title}</p>
                      {isDue && <span className="w-2 h-2 bg-eden-500 rounded-full flex-shrink-0" />}
                    </div>
